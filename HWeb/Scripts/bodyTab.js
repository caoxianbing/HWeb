...
layui.define(["element","jquery"],function(exports){
	...
	Tab.prototype.render = function() {
		//显示左侧菜单
		var _this = this;
		$(".navBar ul").html('<li class="layui-nav-item layui-this"><a data-url="/Home/Default"><i class="layui-icon" data-icon=""></i><cite>首页</cite></a></li>').append(_this.navBar(dataStr)).height($(window).height()-210);
	...
	//是否点击窗口切换刷新页面
	Tab.prototype.changeRegresh = function(index,url){
        if(changeRefreshStr == "true"){
            $(".clildFrame .layui-tab-item").eq(index).find("iframe")[0].contentWindow.location.reload();
        }
        url && ($(".clildFrame .layui-tab-item").eq(index).find("iframe")[0].contentWindow.location.href = url);
	}
	...
	Tab.prototype.tabAdd = function(_this){
		...
		if(_this.attr("target") == "_blank"){
			...
			if (that.hasTab(_this.find("cite").text()) == -1 && _this.siblings("dl.layui-nav-child").length == 0) {
				...
				var _url;
				if (curmenu["href"].indexOf("?") > 0) {
				    _url = curmenu["href"];
				    iframeLoad = layer.load();
				}
				that.changeRegresh(_this.parent('.layui-nav-item').index(), _url);
				window.sessionStorage.setItem("curmenu", JSON.stringify(curmenu));  //当前的窗口
				element.tabChange(tabFilter, that.getLayId(_this.find("cite").text()));
				that.tabMove(); //顶部窗口是否可滚动
			}
